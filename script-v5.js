...
async function loadPilots(file) {
  try {
    ...
    return { base: null, seat: null, pilots: [] };
  }
}
...
async function calcOne(combo, sn) {
  ...
  const seniors = (data.pilots || [])
    .map((p) => Number(p.seniority))
...
function renderList(base, seat, pilots, sn) {
  ...
  if (total === 0) {
    ...
    if (youRow) {
      youRow.classList.add('highlight-hypo');
      youRow.scrollIntoView({ block: 'center' });
    }
    return;
  }
...
async function main() {
  ...
  document.getElementById('calc').addEventListener('click', async () => {
    ...
    if (!combo) {
      alert('No data for that base/seat');
      setBanner('script-v5 ⚠ no combo');
      return;
    }
    const data = await loadPilots(combo.file);
