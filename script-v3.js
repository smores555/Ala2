...
async function calcOne(combo,sn){
  ...
  const rank=lowerBound(seniors, sn) + (seniors.includes(sn) ? 0 : 0); // rank for display uses <= in your sheet, but table uses this consistently
...
// Render roster and slot the typed seniority into the correct place.
// If it exists -> highlight. If not -> insert synthetic "(you — hypothetical)" row.
function renderList(base, seat, pilots, sn){
  ...
  if(eqIndex !== -1){
    ...
    for(let i=0;i<sorted.length;i++){
      ...
      tr.innerHTML = `<td>${p.seniority??''}</td><td>${p.name??''}</td><td>${i+1}</td><td>${total}</td>`;
      tb.appendChild(tr);
    }
  ...
  // insert synthetic row before the first >= sn
  const insertAt = lowerBound(seniors, sn); // 0..total
  for(let i=0;i<sorted.length;i++){
    if(i===insertAt){
      const tr=document.createElement('tr');
      tr.className='you';
      tr.innerHTML = `<td>${sn}</td><td>(you — hypothetical)</td><td>${i+1}</td><td>${total+1}</td>`;
      tb.appendChild(tr);
    }
    const p=sorted[i];
    const tr=document.createElement('tr');
    tr.innerHTML = `<td>${p.seniority??''}</td><td>${p.name??''}</td><td>${i+1 + (i>=insertAt?1:0)}</td><td>${total+1}</td>`;
    tb.appendChild(tr);
  }
  if(insertAt===sorted.length){
    const tr=document.createElement('tr');
    tr.className='you';
    tr.innerHTML = `<td>${sn}</td><td>(you — hypothetical)</td><td>${total+1}</td><td>${total+1}</td>`;
    tb.appendChild(tr);
  }
  tb.querySelector('tr.you')?.scrollIntoView({block:'center'});
}
